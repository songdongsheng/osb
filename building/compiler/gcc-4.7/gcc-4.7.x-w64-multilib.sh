...
# x86_64-w64-mingw32-windres --target=pe-x86-64 -o bzip2.rc.o bzip2.rc
# x86_64-w64-mingw32-windres --target=pe-i386 -o bzip2.rc.o bzip2.rc
# x86_64-w64-mingw32-gcc -print-multi-os-directory
#
# _WIN32, __MINGW32__, __MSVCRT__, __VERSION__, __i386__
#
# x86_64-w64-mingw32-gcc -march=x86-64 -mtune=generic -dM -E -  < /dev/null | sort
# x86_64-w64-mingw32-gcc -march=core2 -mno-sse3       -dM -E -  < /dev/null | sort
# x86_64-w64-mingw32-gcc -march=amdfam10 -mno-sse4a -mno-3dnow -mno-popcnt -mno-abm -dM -E -  < /dev/null | sort
#
...
export CROSS_PREFIX=${HOME}/gcc-4.7-w64-linux
export CROSS_OBJ_ROOT=${HOME}/tmp/gcc-4.7-w64-linux-obj

export NATIVE_PREFIX=${HOME}/gcc-4.7-w64
export NATIVE_OBJ_ROOT=${HOME}/tmp/gcc-4.7-w64-obj

export PATH=${CROSS_PREFIX}/bin:${NATIVE_PREFIX}/bin:${NATIVE_PREFIX}/bin/32:/usr/sbin:/usr/bin:/sbin:/bin

# -------------------------------- Cleanup --------------------------------
rm -fr ${CROSS_OBJ_ROOT} ${CROSS_PREFIX} ${NATIVE_OBJ_ROOT} ${NATIVE_PREFIX}
mkdir -p  ${CROSS_PREFIX}/${TARGET_TRIPLET}/lib  ${CROSS_PREFIX}/${TARGET_TRIPLET}/lib32
mkdir -p ${NATIVE_PREFIX}/${TARGET_TRIPLET}/lib ${NATIVE_PREFIX}/${TARGET_TRIPLET}/lib32
mkdir -p  ${CROSS_PREFIX}/${TARGET_TRIPLET}/include  ${CROSS_PREFIX}/bin/32
mkdir -p ${NATIVE_PREFIX}/${TARGET_TRIPLET}/include ${NATIVE_PREFIX}/bin/32
...
${PPL_SOURCE_DIR}/configure --prefix=${HOME}/gcc-preq-w64/ppl-0.12 \
    -build=${BUILD_TRIPLET} --host=${TARGET_TRIPLET} \
    --disable-shared \
...
${CLOOG_SOURCE_DIR}/configure --prefix=${HOME}/gcc-preq-w64/cloog-0.16 \
    -build=${BUILD_TRIPLET} --host=${TARGET_TRIPLET} \
    --disable-shared -with-isl=bundled \
    --with-gmp-prefix=${HOME}/gcc-preq-w64/gmp-5.0
...
/bin/rm -fr ${NATIVE_PREFIX}/mingw ${NATIVE_PREFIX}/bin/*${BASE_VER}* ${NATIVE_PREFIX}/share/locale \
        ${NATIVE_PREFIX}/lib/*.dll ${NATIVE_PREFIX}/lib32/*.dll \
        ${NATIVE_PREFIX}/${TARGET_TRIPLET}/lib/*.dll ${NATIVE_PREFIX}/${TARGET_TRIPLET}/lib32/*.dll

cd ${NATIVE_OBJ_ROOT}/gcc/${TARGET_TRIPLET}
/bin/cp 32/libgcc/32/shlib/libgcc_s_sjlj-1.dll      \
        32/libgfortran/.libs/libgfortran-3.dll      \
        32/libgomp/.libs/libgomp-1.dll              \
        32/libquadmath/.libs/libquadmath-0.dll      \
        32/libssp/.libs/libssp-0.dll                \
        32/libstdc++-v3/src/.libs/libstdc++-6.dll   ${NATIVE_PREFIX}/bin/32/
...
########################################################################
cd ${NATIVE_PREFIX}/../ && tar -c gcc-4.7-w64 | xz -vz9 > ${HOME}/gcc-${BASE_VER}-w64_${DATE_STR}.tar.xz
########################################################################
python ${HOME}/googlecode_upload.py \
        -s "gcc 4.7 for windows xp/2003 or later - multilib (64 bit executable files)" \
        -p "i18n-zh" \
        -l "gcc, OpSys-Windows, Type-Executable, 64bit" \
        -u dongsheng.song -w rD4Ny3nV8ES8 \
        ${HOME}/gcc-${BASE_VER}-w64_${DATE_STR}.tar.xz
########################################################################
